...
BOOL CUserPropertyPage::OnInitDialog() 
{
	...
	m_rUserPassword = CUserAppDoc::GetInstance()->GetCurProject()->GetAdminPassword().c_str();
...
void CUserPropertyPage::SaveData( const CString& rIniFilePath )
{
	...
	CUserAppDoc::GetInstance()->GetCurProject()->SaveAdminPassword(m_rUserPassword.operator LPCTSTR());
}