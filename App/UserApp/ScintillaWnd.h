...
public:
	...
	// @cmember Append Text to the Scintilla control
	virtual void AppendText (LPCSTR szText);
	// @cmember Set Text to the Scintilla control
	virtual void SetText (LPCSTR szText);
	// @cmember Get Text from the Scintilla control
	virtual void GetText (CString &strText , const long& lStartPos = 0, const long& lEndPos = 0);
	// @cmember Get Text from the Scintilla control
...
public:
	long GetLineLength(const long& lLineNo);
	bool GetLineText(CString& rText , long lLineNo);
	void SetCurrentLineHiLiting(bool isHiliting, COLORREF bgColor);
