...
{
	public class PlugInServiceSelectBoundsCommand : IDisposable
	{
		const int Port = 4421;
		public string ControlName{ get; set; }
        
        public System.Drawing.PointF MinPoint { get; set; }
        public System.Drawing.PointF MaxPoint { get; set; }
				...
				using (TcpClient tcpClient = new TcpClient("127.0.0.1", Port))
				{
					using (NetworkStream stream = tcpClient.GetStream())
					{
						...
						if (null != dsParam)
						{
                            ...
                            {
                                string str = dsResult.Tables[0].Rows[0]["Value"].ToString().Substring(1, dsResult.Tables[0].Rows[0]["Value"].ToString().Length - 2);
                                string[] oTokens = str.Split(',');
                                if (4 == oTokens.Length)
                                {
                                    MinPoint = new System.Drawing.PointF((float)Convert.ToDouble(oTokens[0]), (float)Convert.ToDouble(oTokens[1]));
                                    MaxPoint = new System.Drawing.PointF((float)Convert.ToDouble(oTokens[2]), (float)Convert.ToDouble(oTokens[3]));
                                }
                            }
