...
{
	public class PlugInServicePickPointCommand : IDisposable
	{
		const int Port = 4421;
					...
					using (NetworkStream stream = tcpClient.GetStream())
					{
						...
						if (null != dsParam)
						{
                            ...
                            if ((1 == dsResult.Tables.Count) && (1 == dsResult.Tables[0].Rows.Count))
                            {
                                ...
                                if (2 == oTokens.Length)
                                {
                                    Point = new System.Drawing.PointF((float)Convert.ToDouble(oTokens[0]), (float)Convert.ToDouble(oTokens[1]));
                                }
                            }
