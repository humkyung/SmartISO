...
#ifndef __IS_REGEX_H__
#define __IS_REGEX_H__

#ifdef IS_EXPORTS
	#define IS_EXT_CLASS	__declspec(dllexport)
#else
	#define IS_EXT_CLASS	__declspec(dllimport)
#endif
...
IsRegExp::RxNode IS_EXT_CLASS *IsRxCompile(LPCTSTR Regexp);
int IS_EXT_CLASS IsRxExec(IsRegExp::RxNode *Regexp, LPCTSTR Data, int Len, LPCTSTR Start, IsRegExp::RxMatchRes *Match, unsigned int RxOpt = RX_CASE);
int IS_EXT_CLASS IsRxReplace(LPCTSTR rep, LPCTSTR Src, int len, IsRegExp::RxMatchRes match, LPTSTR *Dest, int *Dlen);
void IS_EXT_CLASS IsRxFree(IsRegExp::RxNode *Node);
